...
if ( fusion_is_element_enabled( 'fusion_recent_posts_carousel' ) ) {

	if ( ! class_exists( 'FusionSC_RecentPostsCarousel' ) ) {
		/**
		 * Shortcode class.
		 *
		 * @since 1.0
		 */
		class FusionSC_RecentPostsCarousel extends Fusion_Element {
			...
			public function __construct() {
				parent::__construct();
				add_filter( 'fusion_attr_recentpostscarousel-shortcode', [ $this, 'attr' ] );
				add_filter( 'fusion_attr_recentpostscarousel-shortcode-section', [ $this, 'section_attr' ] );
				add_filter( 'fusion_attr_recentpostscarousel-shortcode-column', [ $this, 'column_attr' ] );
				add_filter( 'fusion_attr_recentpostscarousel-shortcode-content', [ $this, 'content_attr' ] );
				add_filter( 'fusion_attr_recentpostscarousel-shortcode-slideshow', [ $this, 'slideshow_attr' ] );
				add_filter( 'fusion_attr_recentpostscarousel-shortcode-img', [ $this, 'img_attr' ] );
				add_filter( 'fusion_attr_recentpostscarousel-shortcode-img-link', [ $this, 'link_attr' ] );

				add_shortcode( 'fusion_recent_posts_carousel', [ $this, 'render' ] );

				// Ajax mechanism for query related part.
				add_action( 'wp_ajax_get_fusion_recent_posts_carousel', [ $this, 'ajax_query' ] );
			}
			...
			public function query( $defaults ) {
				...
				if ( ! $recent_posts->have_posts() ) {
					...
					if ( 'yes' == $defaults['thumbnail']){
						$return_data['placeholder'] .= ' There is a chance that you might not have any blog posts with Thumbnail Images.';
					}
			...
			public function render( $args, $content = '' ) {
				...
				$defaults = FusionBuilder::set_shortcode_defaults( self::get_element_defaults(), $args, 'fusion_recent_posts_carousel' );
				...
				if ( ! $recent_posts->have_posts() ) {
					...
					return $return_data['placeholder'];
					...
					if ( 'yes' === $thumbnail && 'date-on-side' !== $layout && ! post_password_required( get_the_ID() ) ) {
						...
						if ( has_post_thumbnail() || $post_video ) {
							...
							if ( has_post_thumbnail() ) {
								...
								$slides .= '<li><a href="' . esc_url( $permalink ) . '" ' . FusionBuilder::attributes( 'recentpostscarousel-shortcode-img-link' ) . '>' . $attachment_image . '</a></li>';
							}
							...
							$slideshow = '<div ' . FusionBuilder::attributes( 'recentpostscarousel-shortcode-slideshow' ) . '><ul ' . FusionBuilder::attributes( 'slides' ) . '>' . $slides . '</ul></div>';
						}
					}
...
// 						$meta_data = fusion_builder_render_post_metadata( 'recent_posts', $this->meta_info_settings );
						...
						foreach($get_the_categories as $cat){
							$the_category = $cat->slug;
							if($defaults['cat_slug'] == $cat->slug && count($defaults['cat_slug']) == 1){
								if($the_categories){
									$the_categories = array();
								}
								$the_categories[] = '<a href="' . get_category_link($cat->term_id) . '" class="cat-' . $cat->slug . '">' . $cat->name . '</a>';
								break;
							}else{
								$the_categories[] = '<a href="' . get_category_link($cat->term_id) . '" class="cat-' . $cat->slug . '">' . $cat->name . '</a>';				
							}
						}
						$meta_data = implode(', ', $the_categories);
						$content  .= '<span ' . FusionBuilder::attributes( 'meta' ) . '>' . $meta_data . '</span>';
					}
					...
					if ( 'yes' === $title ) {
						...
						$content .= '<h2 class="' . $entry_title . '"><a href="' . esc_url( $permalink ) . '">' . get_the_title() . '&nbsp;<span class="fal fa-arrow-right fa-rotate-315 color-'.$the_category.'"></span></a></h2>';
					} else {
					...
					$items .= '<article ' . FusionBuilder::attributes( 'recentpostscarousel-shortcode-column' ) . '>' . $date_box . $slideshow . '<div ' . FusionBuilder::attributes( 'recentpostscarousel-shortcode-content' ) . '>' . $content . '</div></article>';
				}

				$html = '<div ' . FusionBuilder::attributes( 'recentpostscarousel-shortcode' ) . '><section ' . FusionBuilder::attributes( 'recentpostscarousel-shortcode-section' ) . '>' . $items . '</section>' . '<div class="swiper-scrollbar"></div></div>';
			...
			public function attr() {
				...
				$attr = fusion_builder_visibility_atts(
					...
					[
						'class' => 'fusion-recent-posts-carousel fusion-recent-posts fusion-recent-posts-' . $this->recent_posts_counter . ' avada-container layout-' . $this->args['layout'] . ' layout-columns-' . $this->args['columns'] . ' swiper-container',
						'data-swiper-slides-per-view' => $this->args['columns']
					]
				);
			...
			public function section_attr() {
				return [
					'class' => 'fusion-columns columns fusion-columns-' . $this->args['columns'] . ' columns-' . $this->args['columns'] . ' swiper-wrapper',
				];
			}
			...
			public function column_attr() {
				...
				$attr = [
					'class' => 'post fusion-column column col col-lg-' . $columns . ' col-md-' . $columns . ' col-sm-' . $columns . ' swiper-slide',
					'style' => '',
				];
			...
			public function add_scripts() {
				...
				Fusion_Dynamic_JS::enqueue_script(
					'fusion-recent-posts-carousel-swiper',
					get_stylesheet_directory() . '/lib/js/min/swiper.5.2.1-min.js',
					get_stylesheet_directory() . '/lib/js/min/swiper.5.2.1-min.js',
					array( 'jquery' ),
					'1',
					true
				...
				Fusion_Dynamic_JS::enqueue_script(
					'fusion-recent-posts-carousel',
					get_stylesheet_directory() . '/lib/js/min/fusion-recent-posts-carousel-min.js',
					get_stylesheet_directory() . '/lib/js/min/fusion-recent-posts-carousel-min.js',
					array( 'jquery' ),
					'1',
					true
				...
				Fusion_Dynamic_CSS::enqueue_style(
					'fusion-recent-posts-carousel-swiper',
					get_stylesheet_directory_uri() . '/lib/css/swiper.css'
				);
			}
		}
	}

	new FusionSC_RecentPostsCarousel();
}


/**
 *
 * Create our custom Fusion Element
 *
*/
add_action( 'fusion_builder_before_init', 'fusion_recent_posts_carousel' );
function fusion_recent_posts_carousel() {

	$builder_status = function_exists( 'is_fusion_editor' ) && is_fusion_editor();

	fusion_builder_map( 
	    array(
	        'name'            => esc_attr__( 'Recent Posts Carousel', 'fusion-builder' ),
	        'shortcode'       => 'fusion_recent_posts_carousel',
	        'icon'            => 'fusiona-feather',
	        'preview'         =>  get_stylesheet_directory() . '/lib/previews/fusion-recent-posts-carousel-preview.php',
	        'preview_id'      => 'fusion-builder-recent-posts-carousel-preview',
	        ...
	        'params'          => array(
				...
				array(
					...
					'description' => esc_attr__( 'Select the staus(es) of the posts that should be included or leave blank for published only posts.', 'fusion-builder' ),
				...
				array(
					...
					'param_name'  => 'cat_slug',
					'value'       => $builder_status ? fusion_builder_shortcodes_categories( 'category' ) : array(),
					'default'     => '',
					'dependency'  => array(
						array(
							'element'  => 'pull_by',
				...
				array(
					...
					'param_name'  => 'tag_slug',
					'value'       => $builder_status ? fusion_builder_shortcodes_tags( 'post_tag' ) : array(),
					'default'     => '',
					'dependency'  => array(
						array(
							'element'  => 'pull_by',
